 ...
 * [html-webpack-inline-source-plugin](https://github.com/DustinJackson/html-webpack-inline-source-plugin) to inline your src in the resulting HTML file
 * [html-webpack-exclude-src-plugin](https://github.com/jamesjieye/html-webpack-exclude-src-plugin) for excluding src using regular expressions 
 * [html-webpack-include-src-plugin](https://github.com/jharris4/html-webpack-include-src-plugin) for including lists of js or css file paths (such as those copied by the copy-webpack-plugin).
...
If you have any CSS src in webpack's output (for example, CSS extracted
...
- `inject`: `true | 'head' | 'body' | false` Inject all src into the given `template` or `templateContent` - When passing `true` or `'body'` all javascript resources will be placed at the bottom of the body element. `'head'` will place the scripts in the head element.
...
- `htmlWebpackPlugin`: data specific to this plugin
  - `htmlWebpackPlugin.files`: a massaged representation of the
    `srcByChunkName` attribute of webpack's [stats](https://github.com/webpack/docs/wiki/node.js-api#stats)
    object. It contains a mapping from entry point name to the bundle filename, eg:
    ```json
    "htmlWebpackPlugin": {
      "files": {
        "css": [ "main.css" ],
        "js": [ "src/head_bundle.js", "src/main_bundle.js"],
        "chunks": {
          "head": {
            "entry": "src/head_bundle.js",
            "css": [ "main.css" ]
          },
          "main": {
