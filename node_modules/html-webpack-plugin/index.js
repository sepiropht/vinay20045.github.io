...
HtmlWebpackPlugin.prototype.apply = function (compiler) {
  ...
  compiler.plugin('emit', function (compilation, callback) {
    ...
    // Get src
    var src = self.htmlWebpackPluginsrc(compilation, chunks);
    // If this is a hot update compilation, move on!
    // This solves a problem where an `index.html` file is generated for hot-update js files
    // It only happens in Webpack 2, where hot updates are emitted separately before the full bundle
    if (self.isHotUpdateCompilation(src)) {
      return callback();
    }

    // If the template and the src did not change we don't have to emit the html
    var assetJson = JSON.stringify(self.getAssetFiles(src));
    ...
    Promise.resolve()
      ...
      // Allow plugins to make changes to the src before invoking the template
      ...
      .then(function (compilationResult) {
        ...
        return typeof compilationResult !== 'function'
          ? compilationResult
          : self.executeTemplate(compilationResult, chunks, src, compilation);
      })
      // Allow plugins to change the html before src are injected
      .then(function (html) {
        var pluginArgs = {html: html, src: src, plugin: self, outputName: self.childCompilationOutputName};
      ...
      .then(function (result) {
        ...
        var assetTags = self.generateAssetTags(src);
        ...
        return applyPluginsAsyncWaterfall('html-webpack-plugin-alter-asset-tags', true, pluginArgs)
          .then(function (result) {
            ...
            return self.postProcessHtml(html, src, { body: result.body, head: result.head })
              .then(function (html) {
                return _.extend(result, {html: html, src: src});
              });
          });
      })
      // Allow plugins to change the html after src are injected
      .then(function (result) {
        var html = result.html;
        var src = result.src;
        var pluginArgs = {html: html, src: src, plugin: self, outputName: self.childCompilationOutputName};
      ...
      .then(function (html) {
        ...
        compilation.src[self.childCompilationOutputName] = {
          source: function () {
            return html;
          },
          size: function () {
      ...
      .then(function () {
        ...
        return applyPluginsAsyncWaterfall('html-webpack-plugin-after-emit', false, {
          html: compilation.src[self.childCompilationOutputName],
          outputName: self.childCompilationOutputName,
          plugin: self
        }).catch(function (err) {
          console.error(err);
 ...
 * Returns a promise
 */
HtmlWebpackPlugin.prototype.executeTemplate = function (templateFunction, chunks, src, compilation) {
  var self = this;
 ...
 * Returns a promise
 */
HtmlWebpackPlugin.prototype.postProcessHtml = function (html, src, assetTags) {
  var self = this;
  ...
  return Promise.resolve()
    ...
    .then(function () {
      if (self.options.inject) {
        return self.injectsrcIntoHtml(html, src, assetTags);
      } else {
        return html;
      }
    })
...
/*
 * Pushes the content of the given filename to the compilation src
 */
HtmlWebpackPlugin.prototype.addFileTosrc = function (filename, compilation) {
...
HtmlWebpackPlugin.prototype.isHotUpdateCompilation = function (src) {
  return src.js.length && src.js.every(function (name) {
    return /\.hot-update\.js$/.test(name);
  });
};

HtmlWebpackPlugin.prototype.htmlWebpackPluginsrc = function (compilation, chunks) {
  var self = this;
  ...
  var src = {
    ...
    manifest: Object.keys(compilation.src).filter(function (assetFile) {
  ...
  if (this.options.hash) {
    src.manifest = self.appendHash(src.manifest, webpackStatsJson.hash);
    src.favicon = self.appendHash(src.favicon, webpackStatsJson.hash);
  }
  ...
  for (var i = 0; i < chunks.length; i++) {
    ...
    src.chunks[chunkName].size = chunk.size;
    src.chunks[chunkName].entry = entry;
    src.chunks[chunkName].hash = chunk.hash;
    src.js.push(entry);
    ...
    src.chunks[chunkName].css = css;
    src.css = src.css.concat(css);
  }

  // Duplicate css src can occur on occasion if more than one chunk
  // requires the same css.
  src.css = _.uniq(src.css);

  return src;
};

/**
 * Injects the src into the given html string
 */
HtmlWebpackPlugin.prototype.generateAssetTags = function (src) {
  // Turn script files into script tags
  var scripts = src.js.map(function (scriptPath) {
    return {
      tagName: 'script',
      closeTag: true,
      attributes: {
  ...
  var styles = src.css.map(function (stylePath) {
    return {
      tagName: 'link',
 ...
 * Injects the src into the given html string
 */
HtmlWebpackPlugin.prototype.injectsrcIntoHtml = function (html, src, assetTags) {
  ...
  if (src.manifest) {
    html = html.replace(/(<html[^>]*)(>)/i, function (match, start, end) {
      ...
      return start + ' manifest="' + src.manifest + '"' + end;
    });
  }
  return html;
};
 ...
 * asset object
 */
HtmlWebpackPlugin.prototype.getAssetFiles = function (src) {
  var files = _.uniq(Object.keys(src).filter(function (assetType) {
    return assetType !== 'chunks' && src[assetType];
  }).reduce(function (files, assetType) {
    return files.concat(src[assetType]);
  }, []));
  files.sort();
  return files;
};
