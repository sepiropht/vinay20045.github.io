...
class Compilation extends Tapable {
	...
	unseal() {
		...
		this.namedChunks = {};
		this.additionalChunksrc.length = 0;
		this.src = {};
	...
	seal(callback) {
		...
		self.applyPluginsAsyncSeries("optimize-tree", self.chunks, self.modules, function sealPart2(err) {
			...
			self.applyPlugins0("before-module-src");
			self.createModulesrc();
			if(self.applyPluginsBailResult("should-generate-chunk-src") !== false) {
				self.applyPlugins0("before-chunk-src");
				self.createChunksrc();
			}
			self.applyPlugins1("additional-chunk-src", self.chunks);
			self.summarizeDependencies();
			if(shouldRecord)
				self.applyPlugins2("record", self, self.records);

			self.applyPluginsAsync("additional-src", err => {
				if(err) {
					return callback(err);
				}
				self.applyPluginsAsync("optimize-chunk-src", self.chunks, err => {
					if(err) {
						return callback(err);
					}
					self.applyPlugins1("after-optimize-chunk-src", self.chunks);
					self.applyPluginsAsync("optimize-src", self.src, err => {
						if(err) {
							return callback(err);
						}
						self.applyPlugins1("after-optimize-src", self.src);
	...
	createModulesrc() {
		for(let i = 0; i < this.modules.length; i++) {
			...
			if(module.src) {
				Object.keys(module.src).forEach((assetName) => {
					const fileName = this.getPath(assetName);
					this.src[fileName] = module.src[assetName];
	...
	createChunksrc() {
		...
		for(let i = 0; i < this.chunks.length; i++) {
			...
			try {
				...
				if(this.src[file])
					throw new Error(`Conflict: Multiple src emit to the same filename ${file}`);
