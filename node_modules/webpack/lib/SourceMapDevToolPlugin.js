...
class SourceMapDevToolPlugin {
	...
	apply(compiler) {
		...
		compiler.plugin("compilation", compilation => {
			...
			compilation.plugin("after-optimize-chunk-src", function(chunks) {
				let allModules = [];
					...
					chunk.files.filter(ModuleFilenameHelpers.matchObject.bind(undefined, options)).map(function(file) {
						...
						if(asset.__SourceMapDevToolFile === file && asset.__SourceMapDevToolData) {
							...
							for(const cachedFile in data) {
								compilation.src[cachedFile] = data[cachedFile];
								if(cachedFile !== file)
				...
				tasks.forEach(function(task) {
					...
					if(sourceMapFilename) {
						...
						if(currentSourceMappingURLComment !== false) {
							asset.__SourceMapDevToolData[file] = compilation.src[file] = new ConcatSource(new RawSource(source), currentSourceMappingURLComment.replace(/\[url\]/g, sourceMapUrl));
						}
						asset.__SourceMapDevToolData[sourceMapFile] = compilation.src[sourceMapFile] = new RawSource(JSON.stringify(sourceMap));
						chunk.files.push(sourceMapFile);
					} else {
						asset.__SourceMapDevToolData[file] = compilation.src[file] = new ConcatSource(new RawSource(source), currentSourceMappingURLComment
