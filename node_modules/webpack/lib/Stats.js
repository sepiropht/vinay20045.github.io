...
class Stats {
	...
	toJson(options, forToString) {
		...
		const showsrc = optionOrFallback(options.src, true);
		...
		const showCachedsrc = optionOrFallback(options.cachedsrc, true);
		...
		const sortsrc = optionOrFallback(options.srcSort, "");
		...
		if(showsrc) {
			const srcByFile = {};
			obj.srcByChunkName = {};
			obj.src = Object.keys(compilation.src).map(asset => {
				const obj = {
					name: asset,
					size: compilation.src[asset].size(),
					chunks: [],
					chunkNames: [],
					emitted: compilation.src[asset].emitted
				};

				if(showPerformance) {
					obj.isOverSizeLimit = compilation.src[asset].isOverSizeLimit;
				}

				srcByFile[asset] = obj;
				return obj;
			}).filter(asset => showCachedsrc || asset.emitted);

			compilation.chunks.forEach(chunk => {
				chunk.files.forEach(asset => {
					if(srcByFile[asset]) {
						chunk.ids.forEach(id => {
							srcByFile[asset].chunks.push(id);
						});
						if(chunk.name) {
							srcByFile[asset].chunkNames.push(chunk.name);
							if(obj.srcByChunkName[chunk.name])
								obj.srcByChunkName[chunk.name] = [].concat(obj.srcByChunkName[chunk.name]).concat([asset]);
							else
								obj.srcByChunkName[chunk.name] = asset;
						}
					}
				});
			});
			obj.src.sort(sortByField(sortsrc));
		}
		...
		if(showEntrypoints) {
			...
			Object.keys(compilation.entrypoints).forEach(name => {
				...
				obj.entrypoints[name] = {
					...
					src: ep.chunks.reduce((array, c) => array.concat(c.files || []), [])
				};
				if(showPerformance) {
