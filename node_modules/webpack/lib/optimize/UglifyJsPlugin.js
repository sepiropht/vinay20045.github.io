...
class UglifyJsPlugin {
	...
	apply(compiler) {
		...
		compiler.plugin("compilation", (compilation) => {
			...
			compilation.plugin("optimize-chunk-src", (chunks, callback) => {
				const files = [];
				chunks.forEach((chunk) => files.push.apply(files, chunk.files));
				files.push.apply(files, compilation.additionalChunksrc);
				...
				filterdFiles.forEach((file) => {
					...
					let sourceMap;
					try {
						const asset = compilation.src[file];
						if(asset.__UglifyJsPlugin) {
							compilation.src[file] = asset.__UglifyJsPlugin;
							return;
						}
						let input;
						let inputSourceMap;
						...
						if(extractedComments.length > 0) {
							...
							if(commentsFile in compilation.src) {
								// commentsFile already exists, append new comments...
								if(compilation.src[commentsFile] instanceof ConcatSource) {
									compilation.src[commentsFile].add("\n");
									compilation.src[commentsFile].add(commentsSource);
								} else {
									compilation.src[commentsFile] = new ConcatSource(
										compilation.src[commentsFile], "\n", commentsSource
									);
								}
							} else {
								compilation.src[commentsFile] = commentsSource;
							}
						...
						asset.__UglifyJsPlugin = compilation.src[file] = outputSource;
