...
// https://github.com/isaacs/github-flavored-markdown/tree/master/scripts
//
// The `github-flavored-markdown` npm module is not used directly
// because fenced code blocks do not work. It doesn't generate code
// that works with a client-side highlighter.
//
// My modifications integrate highlight.js here to render code
// server-side. All my tweaks are marked with (jwl)
//
// - James Long
...
Showdown.converter = function() {
  ...
  this.makeHtml = function(text, gh) {
    ...
    text = text.replace(/[a-f0-9]{40}/ig, function(wholeMatch, matchIndex) {
      ...
      var left = text.slice(0, matchIndex), right = text.slice(matchIndex);
      if (
        left.match(/@$/) || (left.match(/<[^>]+$/) && right.match(/^[^>]*>/))
      ) {
        return wholeMatch;
      }
    ...
    text = text.replace(
      /([a-z0-9_\-+=.]+)@([a-f0-9]{40})/ig,
      function(wholeMatch, username, sha, matchIndex) {
        if (
          typeof GitHub == "undefined" ||
          typeof GitHub.nameWithOwner == "undefined"
        ) {
    ...
    text = text.replace(
      /([a-z0-9_\-+=.]+\/[a-z0-9_\-+=.]+)@([a-f0-9]{40})/ig,
      function(wholeMatch, repo, sha) {
        return "<a href='http://github.com/" +
          repo +
          "/commit/" +
          sha +
    ...
    text = text.replace(
      /([a-z0-9_\-+=.]+)#([0-9]+)/ig,
      function(wholeMatch, username, issue, matchIndex) {
        if (
          typeof GitHub == "undefined" ||
          typeof GitHub.nameWithOwner == "undefined"
        ) {
    ...
    text = text.replace(
      /([a-z0-9_\-+=.]+\/[a-z0-9_\-+=.]+)#([0-9]+)/ig,
      function(wholeMatch, repo, issue) {
        return "<a href='http://github.com/" +
          repo +
          "/issues/#issue/" +
          issue +
